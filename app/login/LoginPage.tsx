...
/**
 * `/login` page — the entry point for both the email/identifier
 * form and the Google One Tap / FedCM flow. The page is a thin
 * orchestrator that delegates:
 *
 *   - Post-auth navigation  → `./commitLoginRedirect`
 *   - One Tap dismissal state + reason filter → `./useGoogleOneTap`
 *
 * The form field sub-components (`IdentifierField`, `PasswordField`,
 * `SubmitButton`, `GoogleLoginSection`, `LoginCardHeader`) live in
 * this file because each is used exactly once and is tightly
 * coupled to the page's state shape — splitting them across files
 * would just add ceremony without enabling reuse.
 *
 * The default-exported `LoginPage` is a Next.js App Router
 * convention (page entry + `<Suspense>` for `useSearchParams`).
 */
