 ...
 * detail at `/sorotan/{recapId}`; the ID is looked up from
...
export function CoinTickerCard({ coin }: CoinTickerCardProps) {
  ...
  const href = `/sorotan/${COIN_KODE_TO_STORY_ID[coin.kode] ?? ""}`;
  return (
    <Link
      href={href}
      aria-disabled={href === "/sorotan/" ? "true" : undefined}
      tabIndex={href === "/sorotan/" ? -1 : undefined}
